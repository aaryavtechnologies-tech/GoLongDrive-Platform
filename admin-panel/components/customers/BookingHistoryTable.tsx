...
import { CustomerBookingSummary } from '@/types/customer';
...
export function BookingHistoryTable({ data, isLoading }: { data?: CustomerBookingSummary[], isLoading: boolean }) {
                ...
                {data && data.length > 0 ? (
                  data.map((booking) => (
                    <TableRow key={booking.id} className="border-white/10 hover:bg-zinc-900/50">
                      ...
                      <TableCell className="text-zinc-400">{format(new Date(booking.date), 'MMM dd, yyyy')}</TableCell>
                      <TableCell>{booking.driver}</TableCell>
                      <TableCell>{getStatusBadge(booking.status)}</TableCell>
                      <TableCell className="text-right font-medium text-yellow-400">₹{booking.fare}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
