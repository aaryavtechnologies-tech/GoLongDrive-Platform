...
export function DriverTable({ data, isLoading }: DriverTableProps) {
  const columns: ColumnDef<Driver>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="text-sm text-zinc-300">{row.original.vehicle.vehicleNumber}</span>
          <span className="text-xs text-zinc-500">{row.original.vehicle.brand} {row.original.vehicle.model} ({row.original.vehicle.vehicleType})</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      header: 'Registered',
      cell: ({ row }) => <span className="text-sm text-zinc-400">{format(new Date(row.original.joinedDate), 'MMM dd, yyyy')}</span>,
    },
    {
      id: 'actions',
