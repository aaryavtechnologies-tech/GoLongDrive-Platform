...
import { usePayments, useRevenueStats } from '@/hooks/useFinance';
import { PaymentFilters as FilterTypes } from '@/types/payment';
...
import { Search, ListFilter, Download, CreditCard, FilterX } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function PaymentsPage() {
  ...
  const { data: statsData, isLoading: isStatsLoading } = useRevenueStats();
  const { data, isLoading, isError, refetch } = usePayments({ ...filters, page, limit });

  if (isError) {
    return <ErrorBoundary error={new Error('Failed to load payments')} reset={refetch} />;
  }
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">Monitor revenue, payments, refunds, and driver settlements.</p>
        </div>
      ...
      <RevenueCards stats={statsData} isLoading={isStatsLoading} />
      <RevenueCharts />
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between bg-zinc-950 p-4 rounded-lg border border-white/5 shadow-sm">
          <div className="flex flex-1 flex-col sm:flex-row flex-wrap gap-4 w-full">
            <div className="relative w-full sm:w-[320px]">
              ...
              <Input
                placeholder="Search payment ID, booking, customer..."
                value={searchTerm}
            ...
            >
              ...
              <SelectContent className="bg-zinc-950 border-zinc-800 text-white">
                ...
                <SelectItem value="Refunded" className="text-zinc-400">Refunded</SelectItem>
            ...
            <Select 
              ...
              onValueChange={(v: any) => { setFilters({ ...filters, method: v }); setPage(1); }}
            >
              <SelectTrigger className="w-full sm:w-[160px] bg-zinc-900 border-zinc-800 text-white">
